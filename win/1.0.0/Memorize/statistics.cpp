...
void StatisticsWidget::adjustChartRange()
{
    if (accuracySeries->count() > 0) {
        ...
        // 设置X轴范围
        static_cast<QDateTimeAxis*>(chart->axisX())->setMin(minDate);
        static_cast<QDateTimeAxis*>(chart->axisX())->setMax(maxDate);

        // 确保Y轴显示0-100%范围
        static_cast<QValueAxis*>(chart->axisY())->setRange(0, 100);
    }
}
